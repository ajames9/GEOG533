...
```{r}
print("Coefficients are in the hypothesized direction ")
```
...
```{r}
print("In conclusion VIFs correlated, size of lot is insignificant")
```
...
```{r}
price <- (1500*23.4)-(6000*9.5)+(40000*1.57)
price
```
...
m <- lm(df$price ~ df$bedrooms + df$bathroom + df$datebuilt + df$garage + df$fireplace + df$floorarea + df$detached)
summary(m)
print("Bedrooms and bathrooms are insignificant, therefore they can be removed")
```
...
```{r}
m <- lm(df$price ~ df$datebuilt + df$garage + df$fireplace + df$floorarea + df$detached)
summary(m)
```
...
summary(m)
print("Age is insignificant and can be removed")
```
